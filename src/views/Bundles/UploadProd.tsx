...
export const AssetsUploadProd = () => {
    ...
    const saveHistory = (lastSave: string) => {
        let previousHistory = localStorage.getItem('history-prod');
        if (previousHistory) {
            previousHistory += lastSave + '\n';
            localStorage.setItem('history-prod', previousHistory);
            setHistory(previousHistory);
        } else {
            localStorage.setItem('history-prod', lastSave + '\n');
            setHistory(lastSave + '\n');
        }
    }
            ...
            if (dropzone != null)
            {
                for (let i = 0; i < dropzone.files.length; i++)
                {
                    ...
                    s3.upload({
                        ...
                        Key: `assetsbundles/prod/${season}/interacions/${interaction}/${dropzone.files[i].name}`,
                        ...
                        } else
                        {
                            ...
                            writeText(`https://dev-update-manager.s3.amazonaws.com/assetsbundles/prod/${season}/interacions/${interaction}/${dropzone.files[i].name}`).then(() => {
                                saveHistory(`https://dev-update-manager.s3.amazonaws.com/assetsbundles/prod/${season}/interacions/${interaction}/${dropzone.files[i].name}`)
    ...
    useEffect(() => {
        const data = localStorage.getItem('history-prod');
        if (data) {
            setHistory(data);
        }
    }, [])
...
export default AssetsUploadProd;
